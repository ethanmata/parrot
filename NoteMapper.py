...
# TODO: Convert to binary search. It's 2:52 AM and I'd rather not right now.
def find_closest_note(freq):
   dist = math.inf
   note = None
   for i in range(len(freqs)):
      if abs(freqs[i] - freq) < dist:
         note = labels[i]
         dist = abs(freqs[i] - freq)
      else:
         return note
   return note
      

# TODO: Add a proper exception. 2:54 am, ya know?
def label_to_freq(label):
   if label in labels:
      return freqs[labels.index(label)]
   print("Warning: Improper note format")
   
   
# TODO: Convert to binary search. It's 2:56 AM and I've already borrowed most of this code from above.
def snap_to_freq(freq):
   dist = math.inf
   note = None
   for i in range(len(freqs)):
      if abs(freqs[i] - freq) < dist:
